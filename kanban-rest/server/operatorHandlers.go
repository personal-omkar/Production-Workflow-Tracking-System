...
func GetAllOperator(w http.ResponseWriter, r *http.Request) {
	resp, err := http.Get(DBURL + "/get-all-operator")
	if err != nil {
...
func GetOperatorByParam(w http.ResponseWriter, r *http.Request) {
	...
	resp, err := http.Get(DBURL + "/get-operator-by-param?key=" + key + "&value=" + value)
	if err != nil {
		slog.Error("%s - error - %s", "Error making GET request", err)
		utils.SetResponse(w, http.StatusInternalServerError, "Failed to find record")
		return
...
func CreateNewOrUpdateExistingOperator(w http.ResponseWriter, r *http.Request) {
	...
	url := DBURL + "/create-new-or-update-existing-operator"
	client := &http.Client{}
...
func GetAllOperatorBySearchAndPagination(w http.ResponseWriter, r *http.Request) {
	...
	resp, err := http.Post(DBURL+"/get-all-operator-by-search-pagination", "application/json", bytes.NewBuffer(body))
	if err != nil {
