...
var echartsHorizontalStackedChartInit = function () {
  const chartEl = document.querySelector('.echart-horizontal-stacked-chart-example');
  const monthPicker = document.getElementById('vendor-status-month');

  if (!chartEl || !monthPicker) return;

  const chart = window.echarts.init(chartEl);

  function fetchAndRenderChart(monthStr) {
    fetch('/get-vendor-order-status-this-month', {
      method: 'POST',
      ...
      .then(data => {
        ...
        const pending = data.map(item => Number(item.pending));
        const approved = data.map(item => Number(item.approved));
        const rejected = data.map(item => Number(item.rejected));
        const inProgress = data.map(item => Number(item.in_progress));
        const quality = data.map(item => Number(item.quality));
        const packing = data.map(item => Number(item.packing));
        const completed = data.map(item => Number(item.completed));
        ...
        const getDefaultOptions = () => ({
          ...
          legend: {
            data: ['Pending', 'Approved', 'Rejected', 'In Progress', 'Quality', 'Packing', 'Completed'],
          ...
          series: [
            { name: 'Pending', type: 'bar', stack: 'total', label: { show: true }, emphasis: { focus: 'series' }, data: pending },
            { name: 'Approved', type: 'bar', stack: 'total', label: { show: true }, emphasis: { focus: 'series' }, data: approved },
            { name: 'Rejected', type: 'bar', stack: 'total', label: { show: true }, emphasis: { focus: 'series' }, data: rejected },
            { name: 'In Progress', type: 'bar', stack: 'total', label: { show: true }, emphasis: { focus: 'series' }, data: inProgress },
            { name: 'Quality', type: 'bar', stack: 'total', label: { show: true }, emphasis: { focus: 'series' }, data: quality },
            { name: 'Packing', type: 'bar', stack: 'total', label: { show: true }, emphasis: { focus: 'series' }, data: packing },
            { name: 'Completed', type: 'bar', stack: 'total', label: { show: true }, emphasis: { focus: 'series' }, data: completed }
          ],
          grid: {
            right: 15,
            left: 5,
        ...
        echartSetOption(chart, utils.getData(chartEl, 'options'), getDefaultOptions);
      })
      .catch(error => {
        console.error('Error loading vendor status chart:', error);
      });
  }

  function supportsNativeMonthInput() {
    const input = document.createElement('input');
    input.setAttribute('type', 'month');
    return input.type === 'month' && typeof input.valueAsDate !== 'undefined';
  }

  function setupMonthPicker() {
    const today = new Date();
    const currentMonth = today.toISOString().slice(0, 7); // "YYYY-MM"
    monthPicker.setAttribute('type', 'text');

    if (supportsNativeMonthInput()) {
      monthPicker.setAttribute('type', 'month');
      monthPicker.max = currentMonth;
      monthPicker.value = currentMonth;

      const wrapper = monthPicker.closest('.form-group');
      if (wrapper) {
        wrapper.addEventListener('click', (e) => {
          if (e.target !== monthPicker) {
            monthPicker.focus();
            try {
              monthPicker.dispatchEvent(new MouseEvent('mousedown', { bubbles: true }));
              monthPicker.dispatchEvent(new MouseEvent('click', { bubbles: true }));
            } catch (err) {
              console.warn('Could not trigger month input:', err);
            }
          }
        });
      }
      ...
      if (wrapper) {
        wrapper.addEventListener('click', (e) => {
          if (e.target !== monthPicker && monthPicker._flatpickr) {
            monthPicker._flatpickr.open();
          }
        });
      }
    }
