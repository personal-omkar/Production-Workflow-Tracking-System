...
type OperatorManagement struct {
	OperatorId string
	Username   string
	UserType   string
	UserID     string
...
func (u *OperatorManagement) Build() string {
	...
	OperatorTable.CardFooter = Pagination.Build()
	...
	// Create Dialogue
	AddOperatorModel := s.ModelCard{
		ID:      "AddOperatorModel",
		Type:    "modal-m",
		Heading: "Add Operator",
		Form: s.ModelForm{FormID: "AddOperatorModel",
			FormAction: "", Footer: s.Footer{CancelBtn: true, Buttons: []s.FooterButtons{{BtnType: "button", DataSubmitName: "AddOperatorModel", BtnID: "add-operator-submit", Text: "Save"}}},
			Dropdownfield: []s.DropdownAttributes{
				{Label: "Status", Name: "Status", DataType: "bool", Options: statusopt},
			},
			Inputfield: []s.InputAttributes{
				{Type: "text", Name: "OperatorName", DataType: "string", ID: "OperatorName", Label: `Operator Name`, Width: "w-100", Required: true},
				{Type: "text", Name: "OperatorCode", DataType: "string", ID: "OperatorCode", Label: `Operator Code`, Width: "w-100", Required: true},
			},
		},
	}
	...
	//js
		function pagination(pageNo) {
			...
			fetch("/operator-search-pagination", {
				method: "POST",
				headers: {
   	...
   	$(document).ready(function() {
		...
		function removeQueryParams() {
			...
			window.history.replaceState({}, document.title, newUrl);
					...
					$(document).on("click", "#edit-Operator-btn", function(event) {  
					var data = JSON.parse($(this).closest("tr").attr("data-data"));
					// Get modal content for the clicked user
					$.get("/operator-management-card?id=" + data.ID, function(response) {
						let modal = $("#EditOperatorModel");
					...
					$(document).on("click","#add-operator-submit,#edit-operator-submit",function(){
					...
					if (validated){
						...
						$("[data-group='"+group+"'").find("[data-name]").each(function(){
							...
							if ($(this).is("select")){
								...
								$.post("/create-new-or-update-existing-operator", JSON.stringify(result), function() {}, 'json')
									.done(function(response, textStatus, jqXHR) {
										 
										 window.location.href = "/operator-management?status=" +response.code+"&msg="  +response.message;
									})
									.fail(function(xhr, status, error) {
										 data = JSON.parse(xhr.responseText);
										window.location.href = "/operator-management?status=" +data.code+"&msg="  +data.message;
									});
			...
			$(document).on("click", "#importOperators", function(event) {
				
					switch (event.target.id) {
						case 'importOperators':
						$('#om-import-file').trigger('click');
							break;
					}
				});
				$(document).on("change", '#om-import-file', function(e) {
							const url = "/import-operator-data"
							if (url) {
								...
								if (file) {
									...
									$.ajax({
										...
										success: function(data) {
											window.location.href = "/operator-management?status=200&msg=File uploaded successfully"
										},
	...
	html.WriteString(OperatorTable.Build())
	html.WriteString(AddOperatorModel.Build())
