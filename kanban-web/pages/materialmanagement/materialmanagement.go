...
func (u *MaterialManagement) Build() string {
	...
	MaterialTable.CardHeadingActions.Component = []s.CardHeadActionComponent{
		{
			...
			ComponentType: s.ActionComponentElement{
				ModelButton: s.ModelButtonAttributes{Type: "button", Text: "Add New Raw Material", ModelID: "#AddMaterialModel"}},
			Width: "col-4",
		}, {
			ComponentName: "button",
			ComponentType: s.ActionComponentElement{
				Button: s.ButtonAttributes{ID: "importMaterials", Name: "importMaterials", Type: "button", Text: `Import<input style="display:none" class="clickable btn btn-success" type="file" id="om-import-file" name="file" accept=".xlsx">`, Class: "bg-transparent", CSS: "color: #871a83; border: 1px solid #871a83;"},
			},
			Width: "col-3",
		},
		{
	...
	MaterialTable.CardFooter = Pagination.Build()
	...
	// Create Dialogue
	AddMaterialModel := s.ModelCard{
		ID:      "AddMaterialModel",
		Type:    "modal-m",
		Heading: "Add Raw Material",
		Form: s.ModelForm{FormID: "AddMaterialModel",
			FormAction: "", Footer: s.Footer{CancelBtn: true, Buttons: []s.FooterButtons{{BtnType: "button", DataSubmitName: "AddMaterialModel", BtnID: "add-material-submit", Text: "Save"}}},
			Dropdownfield: []s.DropdownAttributes{
				{Label: "Status", Name: "Status", DataType: "bool", Options: statusopt},
			},
			Inputfield: []s.InputAttributes{
				{Type: "text", Name: "Description", DataType: "string", ID: "Description", Label: `Description`, Width: "w-100", Required: true},
				{Type: "text", Name: "SCADACode", DataType: "string", ID: "SCADACode", Label: `SCADA Code`, Width: "w-100", Required: true},
				{Type: "text", Name: "SAPCode", DataType: "string", ID: "SAPCode", Label: `SAP Code`, Width: "w-100", Required: true},
				{Type: "text", Name: "Comment", DataType: "string", ID: "Comment", Label: `Comment`, Width: "w-100", Required: false},
			},
		},
	}
	...
	//js
			function pagination(pageNo) {
				...
				fetch("/material-search-pagination", {
					method: "POST",
					headers: {
		...
		$(document).ready(function() {
			...
			function removeQueryParams() {
				...
				window.history.replaceState({}, document.title, newUrl);
						...
						$(document).on("click", "#edit-Material-btn", function(event) {  
						var data = JSON.parse($(this).closest("tr").attr("data-data"));
						// Get modal content for the clicked user
						$.get("material-management-card?id=" + data.ID, function(response) {
							let modal = $("#EditMaterialModel");
							
							$("#additional-content").html(response)
							// Show the modal after updating/appending
							$("#EditMaterialModel").modal("show");
						}, 'json');
				...
				$(document).on("click","#add-material-submit,#edit-material-submit",function(){
				...
				$.post("/create-new-or-update-existing-material", JSON.stringify(result), function() {}, 'json')
					.done(function(response, textStatus, jqXHR) {
							
							window.location.href = "/material-management?status=" +response.code+"&msg="  +response.message;
					})
					.fail(function(xhr, status, error) {
							data = JSON.parse(xhr.responseText);
						window.location.href = "/material-management?status=" +data.code+"&msg="  +data.message;
					});		
					
				}
			})	
		});
		$(document).on("click", "#importMaterials", function(event) {
			...
			$(document).on("change", '#om-import-file', function(e) {
						const url = "/import-material-data"
						if (url) {
							...
							if (file) {
								...
								$.ajax({
									...
									success: function(data) {
										window.location.href = "/material-management?status=200&msg=File uploaded successfully"
									},
	...
	`
	html.WriteString(MaterialTable.Build())
	html.WriteString(AddMaterialModel.Build())
