...
import (
	...
	"irpl.com/kanban-web/pages/qualitytesting"
	s "irpl.com/kanban-web/services"
)

func QualityTestingPage(w http.ResponseWriter, r *http.Request) {
	...
	qualityTestingPage := qualitytesting.QualityTesting{}
	...
	page := (&basepage.BasePage{
		...
		Username:   username,
		UserType:   usertype,
		Content:    qualityTestingPage.Build(),
	}).Build()
...
func GetAllQualityTestingKanban(w http.ResponseWriter, r *http.Request) {
	...
	// Fetch all vendors
	vendorResp, err := http.Post(RestURL+"/get-all-vendors-data", "application/json", bytes.NewBuffer(reqJSON))
	if err != nil {
	...
	for _, v := range vendors {
		...
		compoundResp, err := http.Post(RestURL+"/get-quality-compound-data-by-vendor?key=id&value="+strconv.Itoa(v.ID),
...
func GetQualityTestingKanbanForVendor(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	...
	resp, err := http.Post(RestURL+"/get-quality-compound-data-by-vendor?key=id&value="+strconv.Itoa(vendor.ID), "application/json", bytes.NewBuffer(reqjsonValue))
	if err != nil {
		slog.Error("%s - error - %s", "Error making GET request", err)
	}
	defer resp.Body.Close()
...
func QualityKanbanSortAsce(w http.ResponseWriter, r *http.Request) {
	...
	if err := decoder.Decode(&data); err == nil {
		...
		resp, err := http.Post(utils.RestURL+"/get-all-vendors-data", "application/json", bytes.NewBuffer(jsonValue))
		if err != nil {
		...
		if err := decoder.Decode(&searchresult); err == nil {
			...
			for _, i := range searchresult {
				...
				sort.Slice(compound, func(i, j int) bool {
					return compound[i].ModifiedOn.Before(compound[j].ModifiedOn)
				})
				...
				if len(compound) > 0 {
					temptable.ModifiedOn = compound[0].ModifiedOn
				}
			...
			sort.Slice(vendortable, func(i, j int) bool {
				if vendortable[i].ModifiedOn.IsZero() && vendortable[j].ModifiedOn.IsZero() {
					return false
				} else if vendortable[i].ModifiedOn.IsZero() {
					return false
				} else if vendortable[j].ModifiedOn.IsZero() {
					return true
				}
				return vendortable[i].ModifiedOn.Before(vendortable[j].ModifiedOn)
			})
		...
		<!--!html-->`
			...
			qtyTesting.BodyTables.ColumnsWidth = []string{"col-1", "col-1", "col-1", "col-9 d-flex flex-wrap w-100 "}
...
func QualityKanbanSortDesc(w http.ResponseWriter, r *http.Request) {
	type SearchRequest struct {
		Criteria map[string]string `json:"Criteria"`
	}
	type TableRequest struct {
	...
	if err := decoder.Decode(&data); err == nil {
		...
		if err := decoder.Decode(&searchresult); err == nil {
			...
			for _, i := range searchresult {
				...
				// Sort by QualityDoneTime in ascending order
				sort.Slice(compound, func(i, j int) bool {
					return compound[i].ModifiedOn.After(compound[j].ModifiedOn)
				})
			...
			sort.Slice(vendortable, func(i, j int) bool {
				...
				return vendortable[i].ModifiedOn.After(vendortable[j].ModifiedOn)
			})
