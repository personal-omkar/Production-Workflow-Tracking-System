...
import (
	...
	"irpl.com/kanban-web/pages/orderhistorypage"
	s "irpl.com/kanban-web/services"
)

func orderHistoryPage(w http.ResponseWriter, r *http.Request) {
	...
	historyPagePage := orderhistorypage.OrderHistory{
		Username: username,
		UserID:   userID,
		UserType: usertype,
	}
...
func GetOrderDetailsForHistory(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		u.SetResponse(w, http.StatusInternalServerError, "Failed to read response body")
		return
	}

	u.SetResponse(w, http.StatusOK, string(responseBody))
}

func SearchCustomerOrderDetails(w http.ResponseWriter, r *http.Request) {
	...
	var customerOrderDetails []*m.CustomerOrderDetails
	...
	if err := decoder.Decode(&data); err == nil {
		...
		if err != nil {
			...
			u.SetResponse(w, http.StatusInternalServerError, "Failed to marshal table condition")
			return
		}

		resp, err := http.Post(u.RestURL+"/get-customer-order-details", "application/json", bytes.NewBuffer(jsonValue))
		if err != nil {
			slog.Error("%s - error - %s", "Error making POST request", err)
			u.SetResponse(w, http.StatusInternalServerError, "Failed to fectch order details")
			return
		}
		defer resp.Body.Close()
		...
		if err := decoder.Decode(&searchresult); err == nil {
			for _, i := range searchresult {
				...
				customerOrderDetails = append(customerOrderDetails, &temp)
			}
		...
		<!--!html-->`
			...
			odcard.BodyTables.Data = customerOrderDetails
			...
			odcard.BodyTables.Tools = tabletool
			...
			u.SetResponse(w, http.StatusInternalServerError, "Fail: failed to create Pagination")
		}

	} else {

		u.SetResponse(w, http.StatusInternalServerError, "Fail: failed to fetch order record")
		slog.Error("%s - error - %s", "failed to fetch order record", err.Error())
	}

}

func GetOrderDetailsForHistoryMobileAPI(w http.ResponseWriter, r *http.Request) {
	...
	u.SetResponse(w, resp.StatusCode, string(responseBody))
