...
import (
	...
	operatormanagement "irpl.com/kanban-web/pages/operatorManagement"
	s "irpl.com/kanban-web/services"
)

func OperatorManagmentPage(w http.ResponseWriter, r *http.Request) {
	...
	operatorManagementPage := operatormanagement.OperatorManagement{
		Username: username,
		UserID:   userID,
		UserType: usertype,
	}
	...
	page := (&basepage.BasePage{
		...
		Username:   username,
		UserType:   usertype,
		Content:    operatorManagementPage.Build(),
	}).Build()
...
func CreateNewOrUpdateExistingOperator(w http.ResponseWriter, r *http.Request) {
	...
	resp, err := http.Post(utils.RestURL+"/create-new-or-update-existing-operator", "application/json", bytes.NewBuffer(jsonValue))
	if err != nil {
...
func operatormanagmentDialog(w http.ResponseWriter, r *http.Request) {
	id := r.URL.Query().Get("id")
	operatorManagementPage := operatormanagement.OperatorManagement{
		OperatorId: id,
	}
	card := operatorManagementPage.CardBuild()
...
func ImportOperatorMasterData(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		sysLog := m.SystemLog{
			Message:     "ImportOperatorMasterData: Invalid file uploaded! Failed to read file",
			MessageType: "ERROR",
			IsCritical:  true,
	...
	records, err := ParseOperatorCSV(file)
	if err != nil {
		sysLog := m.SystemLog{
			Message:     "ImportOperatorMasterData: Invalid file uploaded! Failed to parse file",
			MessageType: "ERROR",
			IsCritical:  true,
			CreatedBy:   "system_logger",
		}
	...
	for _, rec := range records {
		...
		if strings.TrimSpace(rec.OperatorCode) == "" && strings.TrimSpace(rec.OperatorName) == "" {
			sysLog := m.SystemLog{
				Message:     "ImportVendorMasterData: Skipped record! Empty operator code and operator name",
				MessageType: "INFO",
				IsCritical:  true,
				CreatedBy:   "system_logger",
			}
		...
		if strings.TrimSpace(rec.OperatorCode) == "" {
			sysLog := m.SystemLog{
				Message:     "ImportVendorMasterData: Skipped record! Empty operator code for operator " + rec.OperatorName,
				MessageType: "INFO",
				IsCritical:  true,
				CreatedBy:   "system_logger",
			}
		...
		if strings.TrimSpace(rec.OperatorName) == "" {
			sysLog := m.SystemLog{
				Message:     "ImportVendorMasterData: Skipped record! Empty operator name for operator " + rec.OperatorName,
				MessageType: "INFO",
				IsCritical:  true,
				CreatedBy:   "system_logger",
			}
		...
		rawQuery.Type = "Operator"
		rawQuery.Query = `SELECT * FROM operator WHERE operator_code = '` + rec.OperatorCode + `';`
		rawQuery.RawQry(&Operator)

		if len(Operator) != 0 {
			var extOperatorMaster = Operator[0]
			extOperatorMaster.OperatorCode = rec.OperatorCode
			extOperatorMaster.OperatorName = rec.OperatorName
			extOperatorMaster.ModifiedBy = userIDStr
			extOperatorMaster.ModifiedOn = time.Now()
			extOperatorMaster.Status = true

			jsonData, err := json.Marshal(extOperatorMaster)
			if err != nil {
				slog.Error("%s - error - %s", "Error in marshalling data:", err)
			}
			resp, err := http.Post(utils.RestURL+"/create-new-or-update-existing-operator", "application/json", bytes.NewBuffer(jsonData))
			if err != nil {
				sysLog := m.SystemLog{
					Message:     "ImportOperatorMasterData: Skipped record! failed to update vendor: " + extOperatorMaster.OperatorName,
					MessageType: "ERROR",
					IsCritical:  true,
					CreatedBy:   "system_logger",
				}
			...
			var extOperatorMaster m.Operator
			extOperatorMaster.OperatorCode = rec.OperatorCode
			extOperatorMaster.OperatorName = rec.OperatorName
			extOperatorMaster.CreatedBy = userIDStr
			extOperatorMaster.CreatedOn = time.Now()
			extOperatorMaster.Status = true
			...
			if err != nil {
				sysLog := m.SystemLog{
					Message:     "ImportOperatorMasterData: Skipped record! failed to create operator: " + rec.OperatorName,
					MessageType: "ERROR",
					IsCritical:  true,
					CreatedBy:   "system_logger",
				}
		...
		// Check the production for operator and link them
		var prodLine []*m.ProdLine
		rawQuery.Type = "ProdLine"
		if rec.LineName != "Quality" && rec.LineName != "Packing" && rec.LineName != "quality" && rec.LineName != "packing" {
			rawQuery.Query = `SELECT * FROM prod_line WHERE name='` + rec.LineName + ` (Heijunka)';`
		} else {
			rawQuery.Query = `SELECT * FROM prod_line WHERE name='` + rec.LineName + `';`
			log.Printf("rawQuery %v", rawQuery.Query)
		}

		rawQuery.RawQry(&prodLine)
		if len(prodLine) != 0 {
			var ProductionLine = prodLine[0]
			ProductionLine.Operator = rec.OperatorName + " (" + rec.OperatorCode + ")"
			ProductionLine.ModifiedBy = userIDStr
			ProductionLine.ModifiedOn = time.Now()
			ProductionLine.Status = true

			jsonData, err := json.Marshal(ProductionLine)
			if err != nil {
				slog.Error("%s - error - %s", "Error in marshalling data:", err)
			}

			resp, err := http.Post(utils.RestURL+"/create-new-or-update-existing-production-line", "application/json", bytes.NewBuffer(jsonData))
			if err != nil {
				sysLog := m.SystemLog{
					Message:     "ImportOperatorMasterData: Operator added but failed to update operator " + rec.OperatorName + "for machine" + rec.LineName,
					MessageType: "INFO",
					IsCritical:  true,
					CreatedBy:   "system_logger",
				}
			...
			sysLog := m.SystemLog{
				Message:     "ImportOperatorMasterData: Operator added failed to find machine " + rec.LineName + " for operator " + rec.OperatorName + "(" + rec.OperatorCode + ")",
				MessageType: "INFO",
				IsCritical:  true,
				CreatedBy:   "system_logger",
			}
	...
	sysLog := m.SystemLog{
		Message:     "ImportOperatorMasterData: Successfully Imported Operator details",
		MessageType: "SUCCESS",
		IsCritical:  false,
		CreatedBy:   "system_logger",
	}
...
func ParseOperatorCSV(file io.Reader) ([]m.CSVOperator, error) {
	...
	for _, row := range rows {
		...
		if firstCol == "operator name" {
			continue
		}
		// Pad rows shorter than expected
		record := m.CSVOperator{
			OperatorName: strings.TrimSpace(row[0]),
			OperatorCode: strings.TrimSpace(row[1]),
...
func OperatorSearchPagination(w http.ResponseWriter, r *http.Request) {
	var req struct {
	...
	// Send POST request
	resp, err := http.Post(RestURL+"/get-all-operator-by-search-pagination", "application/json", bytes.NewBuffer(jsonValue))
	if err != nil {
	...
	var OperatorTable s.TableCard
	tableTools := `<button type="button" class="btn  m-0 p-0" id="edit-Operator-btn" data-bs-toggle="modal"  > 
	<i class="fa fa-edit " style="color: #871a83;"></i> 
   </button>`
	OperatorTable.CardHeading = "Operator Master"
	OperatorTable.CardHeadingActions.Component = []s.CardHeadActionComponent{
		{
	...
	OperatorTable.BodyTables = s.CardTableBody{Columns: []s.CardTableBodyHeadCol{
		{
			Lable:        "Operator Name",
			Name:         `Operator Name`,
			Type:         "input",
	...
	tableBodyHTML := OperatorTable.BodyTables.RenderBodyColumns()
