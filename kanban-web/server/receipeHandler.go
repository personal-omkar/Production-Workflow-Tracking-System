...
import (
	...
	recipemanagement "irpl.com/kanban-web/pages/recipeManagement"
)
...
func CreateRecipe(w http.ResponseWriter, r *http.Request) {
	...
	if err := decoder.Decode(&data); err == nil {
		...
		resp, err := http.Post(RestURL+"/create-new-recipe", "application/json", bytes.NewBuffer(jsonValue))
		if err != nil {
		...
		slog.Error("%s - error - %s", "Failed to update  data in receipe table", err.Error())
	}
}
...
func GetAllRecipe(w http.ResponseWriter, r *http.Request) {
	...
	resp, err := http.Get(RestURL + "/get-all-recipe")
	if err != nil {
...
func GetRecipeByDataKey(w http.ResponseWriter, r *http.Request) {
	...
	resp, err := http.Get(RestURL + "/get-recipe-by-data-key?key=" + key)
	if err != nil {
		slog.Error("%s - error - %s", "Error making GET request", err)
		utils.SetResponse(w, http.StatusInternalServerError, "Failed to find record")
		return
...
func GetRecipeByDataValue(w http.ResponseWriter, r *http.Request) {
	...
	resp, err := http.Get(RestURL + "get-recipe-by-data-value?value=" + value)
	if err != nil {
		slog.Error("%s - error - %s", "Error making GET request", err)
		utils.SetResponse(w, http.StatusInternalServerError, "Failed to find record")
		return
...
func GetRecipeByDataKeyAndValue(w http.ResponseWriter, r *http.Request) {
	key := r.URL.Query().Get("key")
	value := r.URL.Query().Get("value")
	resp, err := http.Get(RestURL + "/get-recipe-by-data-key-and-value?key=" + key + "&value=" + value)
	if err != nil {
		slog.Error("%s - error - %s", "Error making GET request", err)
		utils.SetResponse(w, http.StatusInternalServerError, "Failed to find record")
		return
...
func DeleteRecipe(w http.ResponseWriter, r *http.Request) {
	...
	if err := decoder.Decode(&recipe); err == nil {
		...
		resp, err := http.Post(RestURL+"/delete-recipe-by-id", "application/json", bytes.NewBuffer(jsonValue))
		if err != nil {
			slog.Error("%s - error - %s", "Error making POST request", err)
			utils.SetResponse(w, http.StatusInternalServerError, "Failed to update cold storage data in inventorytable")
			return
		...
		utils.SetResponse(w, http.StatusInternalServerError, "Fail: failed to update record")
		slog.Error("%s - error - %s", "Failed to delete data in recipe table", err.Error())
	}
}

func EditRecipeDialog(w http.ResponseWriter, r *http.Request) {
	var data m.Recipe
	decoder := json.NewDecoder(r.Body)
	if err := decoder.Decode(&data); err == nil {
		recipe := recipemanagement.RecipeManagement{
			CompoundCode: data.CompoundCode,
			CompoundName: data.CompoundName,
		}
		card := recipe.EditDialogBuild()
...
func UpdateRecipe(w http.ResponseWriter, r *http.Request) {
	...
	if err := decoder.Decode(&data); err == nil {
		...
		jsonValue, err := json.Marshal(data)
		if err != nil {
			slog.Error("%s - error - %s", "Error marshaling recipe data", err)
			utils.SetResponse(w, http.StatusInternalServerError, "Failed to marshal recipe data")
			return
		}

		resp, err := http.Post(RestURL+"/update-existing-recipe", "application/json", bytes.NewBuffer(jsonValue))
		if err != nil {
			slog.Error("%s - error - %s", "Error making POST request", err)
			utils.SetResponse(w, http.StatusInternalServerError, "Failed to update recipe data")
			return
		}
		defer resp.Body.Close()
