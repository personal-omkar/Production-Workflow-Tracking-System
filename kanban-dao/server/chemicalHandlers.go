...
// CreateNewOrUpdateExistingChemical create a new chemical entry or update existing chemical
func CreateNewOrUpdateExistingChemical(w http.ResponseWriter, r *http.Request) {
	...
	if err := decoder.Decode(&data); err == nil {
		...
		err := dao.CreateNewOrUpdateExistingChemical(&data)
		if err != nil {
...
func GetChemicalByParam(w http.ResponseWriter, r *http.Request) {
	...
	KbRoot, err := dao.GetChemicalByParam(key, value)
	if err != nil {
		slog.Error("Recordes not found", "error", err.Error())
		http.Error(w, "Failed to encode fetch records", http.StatusInternalServerError)
		return
...
func GetAllChemicalBySearchAndPagination(w http.ResponseWriter, r *http.Request) {
	...
	KbRoot, PaginationResp, err := dao.GetAllChemicalBySearchAndPagination(requestData.Pagination, requestData.Conditions)
	if err != nil {
		slog.Error("Recordes not found", "error", err.Error())
		http.Error(w, "Failed to encode fetch records", http.StatusInternalServerError)
		return
